...
const LatestExecutionCard = ({
  ...
  <Box 
    ...
    style={{width:'100%', background: colorBackgroundLight(), borderRadius: 12, border: `1px solid ${colorKeylineDefault()}`}}
  >
    <Box 
    ...
    >
      <Box 
        ...
        style={{flex: 1, paddingBottom: '4px', marginBottom: '4px' ,borderBottom: `1px solid ${colorKeylineDefault()}`}}
        >
      ...
      <div>
        ...
        {missing && !materializationFailed && !materializationInProgress && !unstartedRun && (
          <Tag intent="warning" icon='status'>Missing</Tag>
        )}
        ...
        <Box flex={{direction:'column', gap:4}}>
          ...
          <Caption style={{color:colorTextLight()}}>Launched by <Link to='/sensors'>my_sensor_name</Link></Caption>
        </Box>
        )}
  ...
  >
    ...
    >
      ...
      <Box>
        ...
        ) : <Caption style={{color:colorTextDisabled()}}>No automation policy defined</Caption>}
      </Box>
    </Box>
  </Box>
)}
...
const ChecksCard = ({
  ...
  >
    ...
    >
      ...
      <div>
        {!definition?.hasAssetChecks && <Caption style={{color:colorTextDisabled()}}>No checks defined</Caption>}
          <Box flex={{direction: 'row', gap: 4, wrap: 'wrap'}}>
              {checkResults?.NOT_EVALUATED && <Tag icon="status" intent="none">{checkResults.NOT_EVALUATED} not executed</Tag>}
              {checkResults?.IN_PROGRESS && <Tag icon="spinner" intent="primary">{checkResults.IN_PROGRESS} running...</Tag>}
              {checkResults?.SUCCEEDED && <Tag icon="check_circle" intent="success">{checkResults.SUCCEEDED} passed</Tag>}
              {checkResults?.WARN && <Tag icon="warning_outline" intent="warning">{checkResults.WARN} warning</Tag>}
              {checkResults?.ERROR && <Tag icon="cancel" intent="danger">{checkResults.ERROR} failed</Tag>}
          </Box>
      </div>
    </Box>
  ...
  const DataFreshnessCard = ({
    ...
    const latestMaterializationDate = latestMaterializationTimestamp && new Date(latestMaterializationTimestamp);
    const lagMinutes = definition?.freshnessPolicy?.maximumLagMinutes;
    const expirationDate = latestMaterializationDate ? new Date(latestMaterializationDate.getTime() + ((lagMinutes || 0) * 60000)) : null;
    const expired = liveData?.freshnessInfo?.currentMinutesLate && liveData?.freshnessInfo?.currentMinutesLate > 0;
    const missing = liveData?.staleStatus === "MISSING"
    return (
      <Box 
      flex={{direction: 'row', gap: 24, alignItems: 'start'}}
      ...
      >
        ...
        {definition && definition.freshnessPolicy && (
          <Box flex={{direction:'column', gap: 4, alignItems:'start'}}>
            ...
            {!missing && (
              <Caption style={{color:colorTextDisabled()}}>
        ...
        {definition && !definition.freshnessPolicy && (
          <Box flex={{direction:'column', gap: 4, alignItems:'start'}}>
            <Caption style={{color:colorTextDisabled()}}>No freshness policy defined</Caption>
          </Box>
        )}
        </div>
      </Box>
    ...
    >
      ...
      >
        ...
        <Box flex={{direction: 'column', gap: 4, wrap: 'wrap'}}>
          <Caption style={{color:colorTextDisabled()}}>Asset has never been materialized</Caption>
        </Box>
        )}
        ...
        {definition && !stale && !missing && (
          <Box flex={{direction:'column', gap: 4, alignItems:'start'}}>
            <Tag intent="success" icon='check_circle'>Up to date</Tag>
            <Caption style={{color:colorTextDisabled()}}>No changes to reconcile</Caption>
          </Box>
        )}
        </Box>
      </Box>
...
const StatusCard = ({
    ...
    <Box 
      ...
      padding={{vertical: 16, horizontal: 16}}
      style={{background: colorBackgroundLight(), borderRadius: 12, border: `1px solid ${colorKeylineDefault()}`}}
    >
      <Box 
        flex={{direction: 'column', gap: 4}}
        style={{flex: 1}}
      ...
      <Box 
        flex={{direction: 'column', gap: 4}}
        padding={{left: 24}}
        style={{flex: 1, borderLeft: `1px solid ${colorKeylineDefault()}`}}
      >
        <Box 
          flex={{direction: 'row', justifyContent: 'space-between'}}
          style={{flex: 1}}
        ...
        <Box flex={{direction: 'row', gap: 4}}>
          {assetNode?.hasAssetChecks ? "Checks" : <Caption style={{color:colorTextLight()}}>No checks defined</Caption>}
        </Box>
      </Box>
...
const DescriptionCard = ({assetNode}:{assetNode: AssetNodeDefinitionFragment}) => {
  ...
  <Box flex={{direction: 'column', gap: 0}}>
    ...
    >
      ...
      />
      <Subtitle1 style={{color: colorTextLight()}}>Description</Subtitle1>
    </Box>
    <Box
    ...
    {assetNode.description ? (
        ...
        <Body style={{display: 'block', paddingTop: '12px', color: colorTextDisabled()}}>No description provided</Body>
      )}
    </Box>
  </Box>
  )}
  ...
  const DefinitionCard = ({assetNode}:{assetNode: AssetNodeDefinitionFragment}) => {
    const [isOpen, setIsOpen] = React.useState(true);
    const repoAddress = buildRepoAddress(assetNode.repository.name, assetNode.repository.location.name);
    const codeVersion = assetNode.opVersion ? assetNode.opVersion : <Body2 style={{color: colorTextDisabled()}}>Not set</Body2>
    const freshnessPolicy = assetNode.freshnessPolicy ? `Data may be no more than ${assetNode?.freshnessPolicy.maximumLagMinutes} minutes old at any time` : <Body2 style={{color: colorTextDisabled()}}>Not set</Body2>
    const partitionDefinition = assetNode?.partitionDefinition ? assetNode?.partitionDefinition?.description : <Body2 style={{color: colorTextDisabled()}}>Not set</Body2>
    const backfillPolicy = assetNode?.backfillPolicy ? assetNode?.backfillPolicy?.description : <Body2 style={{color: colorTextDisabled()}}>Not set. Defaults to multiple runs, with a maximum of 1 partition per run</Body2>
    ...
    const autoMaterializePolicy2 = assetNode?.autoMaterializePolicy ? buildAutomationPolicyDescription(assetNode?.autoMaterializePolicy.rules) : <Body2 style={{color: colorTextDisabled()}}>Not set</Body2>
    ...
    <Box flex={{direction: 'column', gap: 0}}>
      ...
      >
        <Icon 
          ...
          style={{transform: isOpen ? 'rotate(0deg)' : 'rotate(-90deg)'}}
        />
        <Subtitle1 style={{color: colorTextLight()}}>Definition</Subtitle1>
      </Box>
      <Box
      ...
      >
      <table 
        style={{width: '100%', padding:'12px' , background: colorBackgroundLight(), borderRadius: 12, border: `1px solid ${colorKeylineDefault()}`}}
      >
        <tbody>
          <tr>
            <td style={{minWidth:'240px', paddingBottom: '6px', paddingTop: '6px', borderBottom: `1px solid ${colorKeylineDefault()}`}}>
              <Subtitle2>Asset key</Subtitle2>
            </td>
            <td style={{paddingBottom: '6px', paddingTop: '6px', borderBottom: `1px solid ${colorKeylineDefault()}`}}>
              {assetNode.assetKey.path.join(' : ')}
            </td>
          </tr>
          <tr>
            <td style={{paddingBottom: '6px', paddingTop: '6px', borderBottom: `1px solid ${colorKeylineDefault()}`, verticalAlign: 'top'}}>
    ...
    <Box flex={{direction: 'column', gap: 0}}>
      ...
      >
        <Icon 
          ...
          style={{transform: isOpen ? 'rotate(0deg)' : 'rotate(-90deg)'}}
        />
        <Subtitle1 style={{color: colorTextLight()}}>Metadata</Subtitle1>
      </Box>
      <Box
        padding={{vertical: 0, horizontal: 16}}
        style={{display: isOpen ? 'block' : 'none'}}
      ...
      {assetNode.metadataEntries.length > 0 ? (
        ...
        />
          ...
          <Body style={{display: 'block', paddingTop: '12px', color: colorTextDisabled()}}>No metadata provided</Body>
        )}
      </Box>
    </Box>
    )}
    ...
    <Box flex={{direction: 'column', gap: 0}}>
      ...
      >
        <Icon 
          ...
          style={{transform: isOpen ? 'rotate(0deg)' : 'rotate(-90deg)'}}
        />
        <Subtitle1 style={{color: colorTextLight()}}>Column schema</Subtitle1>
      </Box>
      <Box
        padding={{vertical: 0, horizontal: 16}}
        style={{display: isOpen ? 'block' : 'none'}}
      ...
      {metadataEntries.length > 0 ? (
        <Body style={{display: 'block', paddingTop: '12px', color: colorTextDisabled()}}>
          <DagsterTypeSummary type={assetNode.type?.metadataEntries} />
        </Body>
        ) : (
          <Body style={{display: 'block', paddingTop: '12px', color: colorTextDisabled()}}>No schema provided</Body>
        )}
      </Box>
    </Box>
    )}
      ...
      <Box flex={{direction: 'column', gap: 0}}>
        ...
        >
          <Icon 
            ...
            style={{transform: isOpen ? 'rotate(0deg)' : 'rotate(-90deg)'}}
          />
          <Subtitle1 style={{color: colorTextLight()}}>Lineage</Subtitle1>
        </Box>
        <Box
          padding={{vertical: 0, horizontal: 16}}
          style={{display: isOpen ? 'block' : 'none'}}
        >
        {assetNode.metadataEntries.length > 0 ? (
            <Body style={{display: 'block', paddingTop: '12px', color: colorTextDisabled()}}>Metadata</Body>
          ) : (
            <Body style={{display: 'block', paddingTop: '12px', color: colorTextDisabled()}}>No metadata provided</Body>
          )}
        </Box>
