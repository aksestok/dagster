import {Box, Caption, Tag, Tooltip, AnimatedAutomatorIcon, colorTextDisabled, colorTextLight, colorAccentBlue} from '@dagster-io/ui-components';
  ...
  return (
    ...
    >
      <Tooltip
        content={
          paused
            ? <Box flex={{direction:'column'}}><span>Sensor is paused.</span> <span>New materializations will not be triggered by automation policies.</span></Box>
      ...
      >
        <div>
          <Tag icon='automator' animatedIcon={paused} intent={paused ? 'warning' : 'primary'}>
            <Link to="/overview/automaterialize" style={{outline: 'none'}}> my_sensor_name </Link>
          </Tag>
        </div>
      </Tooltip>
  ...
  return (
    ...
    >
      <Tooltip
        content={
          paused
            ? <Box flex={{direction:'column'}}><span>asset_automation_sensor is paused.</span> <span>New materializations will not be triggered by automation policies.</span></Box>
            : 'asset_automation_sensor is running, evaluates every 30s'
        }
        canShow={paused}
      >
      ...
      <Caption style={{color:colorTextLight()}}>
        Targeted by <Link to="/overview/automaterialize" style={{outline: 'none'}}> my_sensor_name </Link>
      </Caption>
    </Box>
  );
};
